...
class CleanUpDatabase extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->entityManager->getConnection()->executeQuery('UPDATE blog_category SET blog_category_name = "Transport", sort_order = 7, active = 1 WHERE blog_category_code = "TRANSPORT"');
        ...
        if ($blogPostComment) {
            $blogPostComment->setComment('Demo blog post comment for API testing.');
        }

        // Country
        $this->entityManager->getConnection()->executeQuery('DELETE FROM country WHERE country_code = "XX"');
        $this->entityManager->getConnection()->executeQuery('UPDATE country SET country_name = "Argentina" WHERE country_code = "AR"');
        $this->entityManager->getConnection()->executeQuery('UPDATE country SET country_name = "Bahamas" WHERE country_code = "BS"');

        // Information
        $this->entityManager->getConnection()->executeQuery('DELETE FROM information WHERE title = "Create information for API testing"');
        ...
        // Region
        $this->entityManager->getConnection()->executeQuery('DELETE FROM region WHERE region_code = "NEW_REGION"');
        $this->entityManager->getConnection()->executeQuery('UPDATE region SET region_name = "Africa", sort_order = 1, active = 1 WHERE region_code = "AFRICA"');
        $this->entityManager->getConnection()->executeQuery('UPDATE region SET region_name = "Europe", sort_order = 4, active = 1 WHERE region_code = "EUROPE"');

        // User
        $this->entityManager->getConnection()->executeQuery('DELETE FROM user WHERE email = "create.user@example.com"');
        $this->entityManager->getConnection()->executeQuery('UPDATE user SET middle_name="Henry", job_title=null, status="active" WHERE email="user2@example.net"');
